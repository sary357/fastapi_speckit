...
@pytest.mark.asyncio
async def test_rate_limit_sixth_comment_returns_429():
    """T026: Sixth comment from same IP within 1 minute should return 429."""
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        # Make 5 comment requests (should all succeed)
        for i in range(5):
            response = await client.post(
                "/api/comments",
